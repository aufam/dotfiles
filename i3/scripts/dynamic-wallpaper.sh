...
PIDFILE=/tmp/dynamic-wallpaper.pid
if [ -f "$PIDFILE" ]; then
	oldpid=$(cat "$PIDFILE")
...
if [ ! -d "$WALLPAPER_COLLECTION" ]; then
	echo "[ERROR] $(date '+%Y-%m-%d %H:%M:%S') $WALLPAPER_COLLECTION does not exist"
	exit 1
fi
...
if [ -z "$WALLPAPER_DIR" ]; then
	echo "[ERROR] $(date '+%Y-%m-%d %H:%M:%S') No wallpaper directory found in $WALLPAPER_COLLECTION"
	exit 1
fi
...
while true; do
	...
	if [ -f "$WALLPAPER" ]; then
		...
		echo "[ERROR] $(date '+%Y-%m-%d %H:%M:%S') $WALLPAPER does not exist"
	fi
